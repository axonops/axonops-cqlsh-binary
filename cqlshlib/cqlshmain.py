...
import cassandra.main as cassandra_main
...
from cqlshlib.cql_util import get_file_encoding_bomsize, is_file_secure
try:
...
CQL_ERRORS = (
    cassandra_main.AlreadyExists, cassandra_main.AuthenticationFailed, cassandra_main.CoordinationFailure,
    cassandra_main.InvalidRequest, cassandra_main.Timeout, cassandra_main.Unauthorized, cassandra_main.OperationTimedOut,
...
class Shell(cmd.Cmd):
    ...
    def __init__(self, hostname, port, color=False,
        ...
        self.session.default_consistency_level = cassandra_main.ConsistencyLevel.ONE
        ...
        self.consistency_level = cassandra_main.ConsistencyLevel.ONE
        self.serial_consistency_level = cassandra_main.ConsistencyLevel.SERIAL
    ...
    def do_describe(self, parsed):
        ...
        DESCRIBE <objname>
          ...
          view (in this order).
        """
        stmt = SimpleStatement(parsed.extract_orig(), consistency_level=cassandra_main.ConsistencyLevel.LOCAL_ONE, fetch_size=self.page_size if self.use_paging else None)
    ...
    def do_consistency(self, parsed):
        ...
        if level is None:
            print('Current consistency level is %s.' % (cassandra_main.ConsistencyLevel.value_to_name[self.consistency_level]))
            return

        self.consistency_level = cassandra_main.ConsistencyLevel.name_to_value[level.upper()]
    ...
    def do_serial(self, parsed):
        ...
        level = parsed.get_binding('level')
        if level is None:
            print('Current serial consistency level is %s.' % (cassandra_main.ConsistencyLevel.value_to_name[self.serial_consistency_level]))
            return

        self.serial_consistency_level = cassandra_main.ConsistencyLevel.name_to_value[level.upper()]
