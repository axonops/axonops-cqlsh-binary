...
from cassandra.main import ConsistencyLevel
from cassandra.query import Statement, SimpleStatement
from cassandra.datastax.graph.graph_types import Vertex, Edge, Path, VertexProperty
from cassandra.datastax.graph.ds_graphson import GraphSON2Reader, GraphSON3Reader
...
_graph_options = (
    ...
    ('graph_read_consistency_level', '''read `ConsistencyLevel <http://docs.datastax.com/en/developer/python-driver/latest/api/cassandra/#ConsistencyLevel>`_ for graph queries (if distinct from session default).
Setting this overrides the native `Statement.consistency_level <http://docs.datastax.com/en/developer/python-driver/latest/api/cassandra/query/#cassandra.query.Statement.consistency_level>`_ for read operations from Cassandra persistence''', 'graph-read-consistency'),
    ('graph_write_consistency_level', '''write `ConsistencyLevel <http://docs.datastax.com/en/developer/python-driver/latest/api/cassandra/#ConsistencyLevel>`_ for graph queries (if distinct from session default).
